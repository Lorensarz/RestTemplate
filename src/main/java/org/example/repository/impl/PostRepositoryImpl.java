...
public class PostRepositoryImpl implements PostRepository {
    ...
    public List<PostEntity> findAll() {
        ...
        try (Connection connection = dataSource.getConnection()) {
            PreparedStatement preparedStatement = connection.prepareStatement(query);
            ResultSet resultSet = preparedStatement.executeQuery();
            return resultSetMapper.toDtoListPost(resultSet);
        } catch (SQLException e) {
            throw new RuntimeException(e);
        }
    }
    ...
    @Override
    public List<TagEntity> findTagsByPost(PostEntity post) {
        String query = "SELECT * FROM post_tag WHERE post_id = ?";
        try (Connection connection = dataSource.getConnection()) {
            PreparedStatement preparedStatement = connection.prepareStatement(query);
            preparedStatement.setObject(1, post.getId());
            ResultSet resultSet = preparedStatement.executeQuery();
            return resultSetMapper.toDtoListTags(resultSet);
        } catch (SQLException e) {
            throw new RuntimeException(e);
        }
    }
